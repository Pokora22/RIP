...
namespace Sisus.Attributes
{
	public class ValueTest : IShowIfEvaluator
	{
		...
		{
			#if DEV_MODE
			Debug.Log("ParseValue("+valueString+")");
			#endif
			...
			if(!string.Equals(valueString, "this", StringComparison.Ordinal))
			{
		...
		private static MemberInfo ParseStaticClassMember(string valueString)
		{
			...
			for(int n = members.Length - 1; n >= 0; n--)
			{
				...
				if(string.Equals(classMember.Name, memberName, StringComparison.Ordinal))
				{
					#if DEV_MODE
		...
		private static Type ParseType(string className)
		{
			...
			for(int t = checkAssembliesOfTypes.Length - 1; t >= 0; t--)
			{
				...
				for(int n = typesInAssembly.Length - 1; n >= 0; n--)
				{
					...
					{
						if(string.Equals(type.Namespace, preferredNamespace, StringComparison.Ordinal))
						{
							return type;
						}
