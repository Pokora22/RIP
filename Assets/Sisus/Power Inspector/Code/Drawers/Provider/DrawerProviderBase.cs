	...
	public abstract class DrawerProviderBase : IDrawerProvider
	{
		...
		public void Prewarm(IInspector inspector)
		{
			...
			if(DrawerPool.Count < 7 && (InspectorUtility.ActiveInspector == null || InspectorUtility.ActiveInspector.State.drawers.Length == 0)) 
			{
				...
				// make sure the Transform component is unfolded, so that all member
				// IDrawer (Position, Rotation, Scale, TransformFloat) get prewarmed
				...
				for(int n = members.Length - 1; n >= 0; n--)
				{
					...
					if(component != null)
					{
						component.SetUnfolded(true, true);
					}
				}
		...
		/// <inheritdoc/>
		public bool TryGetForDecoratorDrawer([NotNull]PropertyAttribute fieldAttribute, [NotNull]Type propertyAttributeType, IParentDrawer parent, LinkedMemberInfo attributeTarget, out IDecoratorDrawerDrawer result)
		{
		...
		public IDrawer GetForField([CanBeNull]object value, [NotNull]Type fieldType, [CanBeNull]LinkedMemberInfo memberInfo, [CanBeNull]IParentDrawer parent, [CanBeNull]GUIContent label, bool readOnly)
		{
			...
			if(memberInfo != null && memberInfo.MemberInfo != null && UseDrawerAttributeUtility.TryGetCustomDrawerForClassMember(this, memberInfo.MemberInfo, out drawerType))
			{
				...
				{
					return GetForPropertyDrawer(drawerType, memberInfo.GetAttribute<PropertyAttribute>(), value, memberInfo, parent, label, readOnly);
				}
				...
				if(attributeCount > 0)
				{
					...
					for(int n = 0; n < attributeCount; n++)
					{
						...
						if(drawerType != null) 
						{
							...
							Debug.Log("<color=green>PropertyDrawer drawers</color> for field "+StringUtils.ToStringSansNamespace(fieldType)+" and attribute "+fieldAttribute.GetType().Name+": "+drawerType.Name);
							#endif
					...
					for(int n = 0; n < attributeCount; n++)
					{
						...
						for(var baseType = fieldType.BaseType; baseType != null; baseType = baseType.BaseType)
						{
							...
							{
								#if DEV_MODE && (DEBUG_GET_FOR_FIELD || DEBUG_GET_FOR_PROPERTY_DRAWER)
								Debug.Log("<color=green>PropertyDrawer drawers</color> for field "+fieldType.Name+" and attribute "+fieldAttribute.GetType().Name+": "+drawerType.Name);
								#endif
			...
			{
				#if DEV_MODE && (DEBUG_GET_FOR_FIELD || DEBUG_GET_FOR_PROPERTY_DRAWER)
				Debug.Log("<color=green>PropertyDrawer drawers</color> for field "+StringUtils.ToStringSansNamespace(fieldType)+": "+StringUtils.ToStringSansNamespace(drawerType));
				#endif
		...
		/// <inheritdoc/>
		[NotNull]
		public IDrawer GetForPropertyDrawer([NotNull]PropertyAttribute fieldAttribute, [CanBeNull]object value, [NotNull]Type fieldType, [CanBeNull]LinkedMemberInfo memberInfo, [CanBeNull]IParentDrawer parent, [CanBeNull]GUIContent label, bool readOnly)
		{
			#if DEV_MODE && PI_ASSERTATIONS
		...
		/// <inheritdoc/>
		[NotNull]
		public IPropertyDrawerDrawer GetForPropertyDrawer(Type drawerType, [CanBeNull]PropertyAttribute fieldAttribute, [CanBeNull]object value, [CanBeNull]LinkedMemberInfo memberInfo, [CanBeNull]IParentDrawer parent, [CanBeNull]GUIContent label, bool readOnly)
		{
