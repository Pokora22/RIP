...
namespace Sisus
{
	...
	public class DefaultDrawerProvider : DrawerProviderBase
	{
		...
		protected override void BuildDictionaries(DrawerProviderData drawersFor)
		{
			...
			#if DEV_MODE
			Debug.Log("UseDrawerAttributeUtility.GetCustomDrawersByClassType ("+ useDrawerOverrides.Count+ "):\n"+StringUtils.ToString(useDrawerOverrides, "\n"));
			#endif
		...
		private void BuildDictionariesForExactTypes(DrawerProviderData drawersFor, Type[] drawerTypes, bool isFallback)
		{
			...
			var propertyDrawersByFieldType = drawersFor.propertyDrawersByFieldType;
			...
			for(int n = drawerTypes.Length - 1; n >= 0; n--)
			{
				...
				for(int a = attributes.Length - 1; a >= 0; a--)
				{
					...
					if(type != null)
					{
						...
						var decoratorDrawerAttribute = attribute as DrawerForDecoratorDrawerAttribute;
						if(decoratorDrawerAttribute != null)
						{
							if(!decoratorDrawers.ContainsKey(type))
							{
								#if DEV_MODE && DEBUG_BUILD_DICTIONARIES_FOR_DECORATOR_DRAWERS
								Debug.Log(type.Name+" handled by "+drawerType.Name);
								#endif

								decoratorDrawers.Add(type, drawerType);
							}
							continue;
						}

						var propertyDrawerAttribute = attribute as DrawerForPropertyDrawerAttribute;
						if(propertyDrawerAttribute != null)
						{
							if(type.IsSubclassOf(Types.PropertyAttribute))
							{
								Dictionary<Type, Type> drawerTypesByFieldType;
								if(!propertyDrawersByAttributeType.TryGetValue(type, out drawerTypesByFieldType))
								{
									drawerTypesByFieldType = new Dictionary<Type, Type>(1);
									propertyDrawersByAttributeType.Add(type, drawerTypesByFieldType);
								}
								...
								if(!drawerTypesByFieldType.ContainsKey(fieldType))
								{
									...
									#if DEV_MODE && DEBUG_BUILD_DICTIONARIES_FOR_PROPERTY_DRAWERS
									Debug.Log(type.Name+" handled by "+drawerType.Name+" with field type "+fieldType.Name);
									#endif
								...
								#if DEV_MODE && PI_ASSERTATIONS
								Debug.Assert(propertyDrawersByAttributeType.ContainsKey(type));
								Debug.Assert(drawerTypesByFieldType.ContainsKey(fieldType));
								#endif
							}
							else
							{
								if(!propertyDrawersByFieldType.ContainsKey(type))
								{
									#if DEV_MODE && DEBUG_BUILD_DICTIONARIES_FOR_PROPERTY_DRAWERS
									Debug.Log(type.Name+" handled by "+drawerType.Name);
									#endif

									propertyDrawersByFieldType.Add(type, drawerType);
								}

								#if DEV_MODE && PI_ASSERTATIONS
								Debug.Assert(propertyDrawersByFieldType.ContainsKey(type));
								#endif
							}
							continue;
						}
		...
		private void BuildDictionariesForInheritedTypes(DrawerProviderData drawerProviderData, Type[] drawerTypes, bool isFallback)
		{
			...
			var propertyDrawersByFieldType = drawerProviderData.propertyDrawersByFieldType;
			...
			for(int n = drawerTypes.Length - 1; n >= 0; n--)
			{
				...
				for(int a = attributes.Length - 1; a >= 0; a--)
				{
					...
					if(decoratorDrawerAttribute != null)
					{
						var subjectTypes = type.GetExtendingNonUnityObjectClassTypes(false);
						for(int t = subjectTypes.Length - 1; t >= 0; t--)
						{
							var subjectType = subjectTypes[t];
						...
						for(int t = subjectTypes.Length - 1; t >= 0; t--)
						{
							...
							if(type.IsSubclassOf(Types.PropertyAttribute))
							{
								...
								if(!drawerTypesByFieldType.ContainsKey(fieldType))
								{
									...
									#if DEV_MODE && DEBUG_BUILD_DICTIONARIES_FOR_PROPERTY_DRAWERS
									Debug.Log(subjectType.Name+" handled by "+drawerType.Name+" with field type "+fieldType.Name+" because it's a base type of "+type.FullName);
									#endif
							...
							else
							{
								if(!propertyDrawersByFieldType.ContainsKey(type))
								{
									...
									#if DEV_MODE && DEBUG_BUILD_DICTIONARIES_FOR_PROPERTY_DRAWERS
									Debug.Log(subjectType.Name+" handled by "+drawerType.Name+" because it's a base type of "+type.FullName);
									#endif
								}
		...
		private void AssignTypeToUseDecoratorDrawerDrawer(DrawerProviderData drawersFor, Type attributeType)
		{
			...
			#if DEV_MODE && DEBUG_BUILD_DICTIONARIES_FOR_DECORATOR_DRAWERS
			Debug.Log(attributeType.Name+" (DecoratorDrawer) with default field type System.Object handled by DecoratorDrawerDrawers.");
			#endif
