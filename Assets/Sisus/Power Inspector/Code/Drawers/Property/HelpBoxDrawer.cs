...
namespace Sisus
{
	[Serializable, DrawerForDecoratorDrawer(typeof(HelpBoxAttribute), true)]
	public sealed class HelpBoxDrawer : DynamicDecoratorDrawerDrawer
	{
		...
		public override void SetupInterface(PropertyAttribute propertyAttribute, Type decoratorDrawerType, IParentDrawer setParent, LinkedMemberInfo attributeTarget)
		{
			...
			Setup(helpBoxAttribute.text, setParent, helpBoxAttribute.messageType, helpBoxAttribute.GetEvaluator(), helpBoxAttribute.minHeight);
		}
		...
		private void Setup(string text, [CanBeNull]IParentDrawer setParent, HelpBoxMessageType setMessageType = HelpBoxMessageType.Info, IShowInInspectorIf showInInspectorEvaluator = null, float setMinHeight = 31f)
		{
			...
			base.Setup(text, typeof(string), null, setParent, GUIContentPool.Create(text), false);
		}
		...
		{
			return showHelpBoxEvaluator == null ? true : showHelpBoxEvaluator.ShowInInspector(containingClassType, containingClassInstance, classMember);
		}
