...
namespace Sisus
{
	...
	public abstract class UnityObjectDrawer<TSelf, TTarget> : ParentDrawer<LinkedMemberInfo>, IUnityObjectDrawer, IDebuggable where TSelf : UnityObjectDrawer<TSelf, TTarget> where TTarget : Object
	{
		...
		public override bool Selectable
		{
			get
			{
				return passedLastFilterCheck && (!HeadlessMode || CanBeSelectedWithoutHeaderBeingSelected) && ShownInInspector;
			}
		}
		...
		protected virtual float PrefixResizerDragHandleHeight
		{
			get
			{
				return PrefixResizer == PrefixResizer.TopOnly ? 5f : 0f;
			}
		}
		...
		protected virtual bool DrawGreyedOut
		{
			get
			{
				return !Enabled || !Editable || (Target != null && HasHideFlag(HideFlags.HideInInspector));
			}
		}
		...
		protected HeaderPartDrawer SelectedHeaderPart
		{
			...
			private set
			{
				...
				if((HeaderPart)selectedHeaderPart != value) { Debug.Log(StringUtils.ToColorizedString("selectedHeaderPart = ", ((HeaderPart)value), " (was: ", ((HeaderPart)selectedHeaderPart), ")")); }
				#endif
		...
		protected virtual void DrawHeaderBase(Rect position)
		{
			...
			if(Event.current.type == EventType.Repaint || EditingComponentDisplayName)
			{
				...
				{
					if(!string.Equals(GUI.GetNameOfFocusedControl(), "Edit Component Display Name", StringComparison.Ordinal))
					{
					...
					if(!string.Equals(setDisplayName, displayNameWas, StringComparison.Ordinal))
					{
						#if DEV_MODE
		...
		protected virtual void DoBuildHeaderToolbar()
		{
			...
			if(HasExecuteMethodIcon)
			{
				AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.MethodInvokerButton, true, true, InspectorUtility.Preferences.graphics.ExecuteIcon, "Invoke methods on target Object.\nCtrl + Click to also list invisible methods.", OnExecuteMethodIconClicked, OnExecuteMethodIconRightClicked));
			}
			...
			AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.Base, false, false, "", null));
			...
			inspector.OnNextLayout(()=>
			{
				...
				{
					Debug.LogError(ToString()+".BuildHeaderPartInfos was called before ready to call Foldable.");
				}
			});
			#endif
			...
			for(int n = headerParts.Count - 1; n >= 0; n--)
			{
				...
				switch(part.Part)
				{
					...
					case HeaderPart.DebugModePlusButton:
						...
						break;
					case HeaderPart.MethodInvokerButton:
		...
		public virtual void AddItemsToOpeningViewMenu(ref Menu menu)
		{
			...
			#if UNITY_EDITOR
			ViewMenuUtility.AddFieldVisibilityItems(ref menu, inspector, MonoScript != null);/*, MonoScript != null && GetType() != typeof(MonoScriptDrawer));*/
			#else
		...
		public override void OnMiddleClick(Event inputEvent)
		{
			...
			Debug.Log(ToString()+".OnMiddleClick("+StringUtils.ToString(inputEvent)+") with HeaderMouseovered="+StringUtils.ToColorizedString(HeaderMouseovered));
			#endif
