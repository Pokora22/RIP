	...
	public class GameObjectDrawer : ParentDrawer<Component[]>, IGameObjectDrawer, IOnProjectOrHierarchyChanged, IEnumerable<IComponentDrawer>
	{
		...
		protected float HeaderHeightOutsideDebugMode
		{
			get
			{
				...
				if(isPrefab)
				{
					return DrawGUI.Active.AssetTitlebarHeight(true);
				}
				#endif
		...
		public virtual void Setup([NotNullOrEmpty]GameObject[] setTargets, [CanBeNull]IParentDrawer setParent, [NotNull]IInspector setInspector)
		{
			...
			if(firstTarget != null)
			{
				...
				// Confusingly, this not only returns true for scene instances of prefabs, but for prefab variants as well.
				// This is why it is important to check IsPartOfPrefabAsset before this.
				else if(PrefabUtility.IsOutermostPrefabInstanceRoot(firstTarget))
				{
					isPrefabInstance = true;

					#if DEV_MODE && DEBUG_IS_PREFAB
					Debug.Log("prefab instance. isOpenInPrefabMode = "+isOpenInPrefabMode);
					#endif
				}
				#if DEV_MODE && DEBUG_IS_PREFAB
				else { Debug.Log("Non-prefab gameobject. isOpenInPrefabMode = "+isOpenInPrefabMode); }
				#endif
			...
			if(targetCount > 0)
			{
				...
				{
					if(!string.Equals(setName, setTargets[n].name, StringComparison.Ordinal))
					{
						setName = "―";
						break;
					}
		...
		protected static void GenerateMemberBuildList(GameObject[] targets, ref List<Component[]> memberBuildList, ref bool componentsOnlyOnSomeObjectsFound, bool showHiddenComponents)
		{
		...
		protected override void DoBuildMembers()
		{
			...
			for(index = 0; index < count; index++)
			{
				...
				#if DEV_MODE && ENABLE_MATERIAL_DRAWER_EMBEDDING && UNITY_EDITOR
				if(Types.Renderer.IsAssignableFrom(drawer.Type))
				{
					...
					for(int m = 0, materialCount = materialsOnAllTargets.Count; m < materialCount; m++)
					{
						...
						var materialDrawer = MaterialDrawer.Create(materialTargets, drawer, inspector);
						index++;
						count++;
		...
		public override bool DrawBody(Rect position)
		{
			...
			for(int n = 0; n < count; n++)
			{
				...
				{
					#if DEV_MODE
					Debug.LogError("GameObject member was null during DrawBody!");
					#endif
		...
		private void UpdateGrayedOut()
		{
			...
			if(gameObject != null)
			{
				...
				grayedOut = !gameObject.activeInHierarchy;
				#if DEV_MODE && DEBUG_GRAYED_OUT
			...
			if(reorderingControl == null)
			{
				...
				for(int n = 0; n < count; n++)
				{
					...
					if(monoScript != null)
					{
						...
						OnNextLayout(()=> AddComponent(monoScript.GetClass(), true, addComponentIndex));
						dropIndex++;
		...
		/// <inheritdoc/>
		public Component[] MoveComponentsUnderComponents(Component[] componentsToMove, Component[] moveUnderneath)
		{
			#if !UNITY_EDITOR
		...
		public virtual void OnSubjectOverDropTarget(MouseDownInfo mouseDownInfo, Object[] draggedObjects)
		{
			...
			Debug.Assert(mouseDownInfo.MouseButtonIsDown);
			#endif
		...
		public override bool OnClick(Event inputEvent)
		{
			...
			Debug.Log("KeyboardControlUtility.KeyboardControl=" + KeyboardControlUtility.KeyboardControl + ", beforeHeaderControlID="+ beforeHeaderControlID+", offset="+(KeyboardControlUtility.KeyboardControl - beforeHeaderControlID));
			#endif
		...
		public override void AddPreviewWrappers(ref List<IPreviewableWrapper> previews)
		{
			...
			for(int n = LastVisibleCollectionMemberIndex; n >= 0; n--)
			{
		...
		private void UpdateLabelText()
		{
			...
			{
				if(!string.Equals(setName, targets[n].name, StringComparison.Ordinal))
				{
					setName = "―";
					break;
				}
		...
		/// <inheritdoc/>
		public IComponentDrawer AddComponent(Type type, bool scrollToShow, int index = -1)
		{
			#if DEV_MODE
		...
		private IComponentDrawer DoAddComponent([NotNull]Type type, bool scrollToShow, int index)
		{
			...
			bool addingAsLastComponent = index == -1 || index + 1 >= memberCount;
			var existingComponentsAtIndex = addingAsLastComponent ? null : members[index - 1].GetValues() as Component[];
