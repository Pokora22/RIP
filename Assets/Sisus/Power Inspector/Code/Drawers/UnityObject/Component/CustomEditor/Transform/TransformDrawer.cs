...
namespace Sisus
{
	...
	public class TransformDrawer : ComponentDrawer, ITransformDrawer
	{
		...
		protected override void Setup(Component[] setTargets, IParentDrawer setParent, GUIContent setLabel, IInspector setInspector)
		{
			if(setParent == null)
			{
				Debug.LogError("Transform had no parent!");
			}
		...
		private void UpdateHeight()
		{
			...
			if(NowUnfolding)
			{
				try
				{
					inspector.OnNextLayout(UpdateHeight);
				}
				catch(NullReferenceException)
				{
					// this happened for some reason when Unity started up
					// and Unity layout contained a Power Inspector window
					// which was showing a Transform
					if(inspector == null)
					{
						inspector = InspectorUtility.ActiveInspector;
						#if DEV_MODE
						Debug.LogError("inspector was null! InspectorUtility.ActiveInspector=="+ (inspector == null ? "null" : inspector.ToString()));
						#endif
					}
				}
			}
