...
namespace Sisus
{
	...
	public class ComponentDrawer : UnityObjectDrawer<ComponentDrawer, Component>, IEditorlessComponentDrawer, IReorderable
	{
		...
		/// <inheritdoc cref="IComponentDrawer.Expandable" />
		...
		protected override void DrawHeaderParts()
		{
			if(createCustomEnabledFlag)
			{
				...
				if(behaviour != null)
				{
					GUI.Toggle(EnabledFlagPosition, behaviour.enabled, GUIContent.none);
				}
			}
		...
		public override void OnMouseUpAfterDownOverControl(Event inputEvent, bool isClick)
		{
			...
			if(isClick && HasEnabledFlag && MouseoveredHeaderPart == HeaderPart.EnabledFlag)
			{
				...
				if(createCustomEnabledFlag)
				{
					...
					#if DEV_MODE && PI_ASSERTATIONS
					Debug.Assert(behaviour != null, "createCustomEnabledFlag was true but target was not a Behaviour");
					#endif

					bool wasEnabled = behaviour.enabled;
					UndoHandler.RegisterUndoableAction(behaviour, wasEnabled ? "Disable Component" : "Enable Component");
