...
namespace Sisus
{
	...
	public abstract class CustomEditorBaseDrawer<TSelf, TTarget> : UnityObjectDrawer<TSelf, TTarget>, ICustomEditorDrawer where TSelf : CustomEditorBaseDrawer<TSelf, TTarget> where TTarget : Object
	{
			...
			if(Platform.EditorMode)
			{
				...
				if(MembersAreVisible)
				{
					...
					if(PrefixResizer != PrefixResizer.Disabled)
					{
						...
						windowRect.y += HeaderHeight;
						windowRect.height = Height - HeaderHeight;

						var resizerPos = position;
						resizerPos.y += HeaderHeight;
						resizerPos.height = 7f;
						resizerPos.x = PrefixLabelWidth - 5f;
						resizerPos.width = 9f;
		...
		public override bool DrawPrefix(Rect position)
		{
			...
			DrawGUI.LayoutSpace(position.height + PrefixResizerDragHandleHeight);
		...
		private bool DrawMembers(Rect position)
		{
			...
			else if(usingInitialEstimatedHeight || NowUnfolding)
			{
				#if DEV_MODE
		...
		private int GetNextEditorControlDown(bool secondAttempt = false)
		{
			...
			{
				//rectWas = new Rect(ControlsLeftMargin, bodyLastDrawPosition.y + DragHandleHeight + ControlsTopMargin - ControlsRowHeight, DrawGUI.InspectorWidth - 18f, ControlsRowHeight);
				rectWas = bounds;
			...
			else
			{
				...
				optimalY = rectWas.yMax + 2f; // + ControlsRowHeight; // 2f is offset between controls (margin x 2)
