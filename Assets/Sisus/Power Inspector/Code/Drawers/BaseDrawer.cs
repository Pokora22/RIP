	...
	public abstract class BaseDrawer : IDrawer, IDisposable, IEquatable<BaseDrawer>
	{
		...
		protected virtual Rect SelectionRect
		{
			get
			{
				...
				//DrawGUI.AddMarginsAndIndentation(ref pos);
				return pos;
				#endif
			}
		}
		...
		protected virtual void BuildContextMenu(ref Menu menu, bool extendedMenu)
		{
			...
			if(extendedMenu)
			{
				...
				if(!(this is ClassDrawer))
				{
					...
					{
						var monoScript = GetValue() as UnityEditor.MonoScript;
						if(monoScript != null)
						{
		...
		protected bool HasOnValueChangedBreakPoint()
		{
			...
			for(int n = list.Length - 1; n >= 0; n--)
			{
				...
				if(method != null && string.Equals(method.Name, "OnValueChangedBreakPoint", StringComparison.Ordinal))
				{
					return true;
				}
			}
		...
		protected bool HasOnValueChangedLogPoint()
		{
			...
			{
				var method = list[n].Method;
				if(method != null && string.Equals(method.Name, "OnValueChangedLogPoint", StringComparison.Ordinal))
				{
					return true;
				}
			}
