...
namespace Sisus
{
	...
	public static class ParentDrawerUtility
	{
		...
		{
			if(subject.ClickToSelectArea.Contains(mousePosition))
			{
			...
			#if DEV_MODE && DEBUG_MOUSEOVER_DETECTION
			Debug.Log(StringUtils.ToColorizedString("DetectMouseover(", subject, "): ", subject.ClickToSelectArea, ".MouseIsOver: ", false));
			#endif
		...
		public static void SetMembers([NotNull]IParentDrawer parent, [NotNull]ref IDrawer[] members, [NotNull]IDrawer[] setMembers, [NotNull]ref IDrawer[] visibleMembers, ref MemberBuildState memberBuildState, ref bool forceVisibleMembersChangedCall, List<IDrawer> updateCachedValuesFor, bool sendVisibilityChangedEvents)
		{
			...
			#if DEV_MODE
			else { Debug.LogWarning("SetMembers("+StringUtils.ToString(setMembers)+") - Won't dispose previouslyVisibleMembers " + StringUtils.ToString(previouslyVisibleMembers)+" because they match new visibleMembers."); }
			#endif
		}
		...
		/// <param name="membersListedVertically"> True to members listed vertically. </param>
		/// <returns> The next selectable drawer up. This may be null. </returns>
		private static IDrawer GetNextInternalSelectableFieldDownInternal(IParentDrawer subject, int column, IDrawer requester, bool membersListedVertically)
		{
			var members = subject.VisibleMembers;
			int count = members.Length;
		...
		public static void GetMemberBuildList([NotNull]IUnityObjectDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, [NotNull]DebugModeDisplaySettings debugModeSettings)
		{
			...
			if(debugModeSettings.ShowFields)
			{
				for(var type = topLevelType; type != null; type = type.BaseType)
				{
					...
					type.GetInspectorViewableFields(ref hierarchy, parentMemberInfo, ref results, bindingFlags, true, true, parentType);
				}
			}
		...
		public static void GetMemberBuildList([NotNull]IParentDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, bool includeHidden, BindingFlags bindingFlags = BindingFlagsDeclaredInstanceAndStatic)
		{
			...
			var showNonSerializedFields = settings.showNonSerializedFields;
		...
		/// <summary>
		/// Builds IDrawer for members in buildList.
		/// NOTE: Members must be grouped by MemberType in order matching InspectorUtility.Preferences.MemberDisplayOrder.
		/// </summary>
		/// <param name="parent">
		/// The parent whose members will be built. </param>
		/// <param name="buildList">
		/// List of LinkedMemberInfos from which to build members. </param>
		/// <param name="results">
		/// [in,out] The results. </param>
				...
				switch(order[t])
				{
					case Member.Field:
						for(; n < count; n++)
						{
							...
							DrawerUtility.TryGetDecoratorDrawer(drawerProvider, linkedInfo, ref membersList, parent);
					...
					case Member.Property:
						for(; n < count; n++)
						{
							...
							var label = linkedInfo.GetLabel();
							var member = drawerProvider.GetForProperty(linkedInfo.GetValue(0), linkedInfo.Type, linkedInfo, parent, label, readOnly);
...
catch { Debug.LogError(StringUtils.ToString(members)); }
		}
