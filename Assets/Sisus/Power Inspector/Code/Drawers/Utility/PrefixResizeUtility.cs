...
namespace Sisus
{
	...
	{
		public const float TopOnlyPrefixResizerHeight = 5f;
		...
		public static float HandleResizing(IUnityObjectDrawer target, out bool prefixResizerMouseovered, float maxHeight, float minPrefixLabelWidth, float maxPrefixLabelWidth)
		{
			...
			if(drawTopSplitter)
			{
				...
				topResizerPos.width = prefixWidth - 5f - topResizerPos.x;
				...
				//right
				topResizerPos.x += topResizerPos.width + 9f;
				topResizerPos.width = target.ClickToSelectArea.width - topResizerPos.x - DrawGUI.IndentWidth - +DrawGUI.MiddlePadding + DrawGUI.RightPadding;
