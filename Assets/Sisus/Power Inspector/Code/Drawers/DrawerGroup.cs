...
namespace Sisus
{
	...
	public sealed class DrawerGroup : ParentDrawer<object>, IReorderableParent
	{
		...
		{
			#if DEV_MODE
			Debug.Log(ToString()+ ".DisposeMembersAndClearVisibleMembers");
			#endif
		...
		public void OnMemberDragNDrop(MouseDownInfo mouseDownInfo, Object[] draggedObjects)
		{
			...
			if(dropIndex >= 0)
			{
				...
				else
				{
					...
					{
						var setMembers = Members;
						var clone = GameObjectDrawer.Create(draggedGameObjectDrawer.GetValues() as GameObject[], this, reordering.MouseoveredDropTarget.Inspector);
