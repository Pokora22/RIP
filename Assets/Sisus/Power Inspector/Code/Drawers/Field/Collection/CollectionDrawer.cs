...
//#define ENABLE_UNFOLD_ANIMATIONS_A //GUI.BeginGroup-based
//#define ENABLE_UNFOLD_ANIMATIONS_B //sets height of each member
#define ENABLE_UNFOLD_ANIMATIONS_C //GUIUtility.ScaleAroundPivot-based
#define ENABLE_UNFOLD_ANIMATIONS_ALPHA
...
namespace Sisus
{
	...
	public abstract class CollectionDrawer<TValue, TSize, TIndex> : ParentFieldDrawer<TValue>, ICollectionDrawer, IReorderableParent where TValue : class, IEnumerable where TSize : struct, IEquatable<TSize> where TIndex : struct, IEquatable<TIndex>
	{
		...
		{
			member.Label = GetIndexBasedLabelForMember(collectionIndex);
		...
		public void OnMemberDragNDrop(MouseDownInfo mouseDownInfo, Object[] draggedObjects)
		{
			...
			bool sameParents = reordering.Parent == this;
			...
			var memberValues = reordering.Drawer.GetValues();
			...
			#if DEV_MODE && DEBUG_REORDERING
			Debug.Log("OnMemberDragNDrop from ["+ fromCollectionFlatIndex + "] to ["+ toCollectionFlatIndex + "]");
			#endif

			var fromIndex = GetCollectionIndex(fromCollectionFlatIndex);

			//remove dragged value from source parent
			if(sameParents)
			{
			...
			{
				reordering.Parent.DeleteMember(reordering.Drawer);
			}
			
			var toIndex = GetCollectionIndex(toCollectionFlatIndex);

			// insert dragged value to target parent
			// question: is there any undo support currently???
			int lastValueIndex = memberValues.Length - 1;
			for(int n = collectionCount - 1; n >= 0; n--)
			{
				var collection = collections[n];
				if(GetCollectionElementCount(collection) >= toCollectionFlatIndex)
				{
					TryInsertAt(ref collection, toIndex, memberValues[Mathf.Min(n, lastValueIndex)]);
					collections[n] = collection;
				}
			}
		...
		public bool SubjectIsReorderable(Object member)
		{
			...
			return member.GetType() == MemberType;
		}
		...
		public override bool DrawBodyMultiRow(Rect position)
		{
			...
			#if ENABLE_UNFOLD_ANIMATIONS && ENABLE_UNFOLD_ANIMATIONS_A
			var groupRect = position;
		...
		protected virtual bool DrawFoldableContent(Rect position)
		{
			...
			DrawGUI.IndentLevel += appendIndentLevel;
			{
				...
				while(visibleMemberIndex < count)
				{
					...
					//if dragged element is currently positioned at this index
					...
					#if ENABLE_UNFOLD_ANIMATIONS && ENABLE_UNFOLD_ANIMATIONS_B
					if(nowUnfolding)
					{
						position.height *= unfoldedness;
					}
					#endif
				...
				if(nowReordering)
				{
					...
					if(drawReorderedRect.width > 0)
					{
						...
						arrowPos.y += 3f;
						GUI.Label(arrowPos, GUIContent.none, "Icon.ExtrapolationContinue");
						drawReorderedRect.y = Cursor.LocalPosition.y + reordering.Drawer.MouseDownCursorTopLeftCornerOffset.y; //add some clamping?
						bgRect.y = drawReorderedRect.y;
						...
						else
						{
							...
							labelWas = GUIContentPool.Create(reordering.Drawer.Label);
							UpdateMemberLabelForIndex(reordering.Drawer, GetCollectionIndex(dropTargetCollectionIndex));
						}
						
						if(reordering.Drawer.Draw(drawReorderedRect))
						{
							dirty = true;
							GUI.changed = true;
						}
