...
namespace Sisus
{
	...
	public class DictionaryEntryDrawer : ParentFieldDrawer<DictionaryEntry>
	{
		...
		public static DictionaryEntryDrawer Create(DictionaryEntry value, Type keyType, Type valueType, LinkedMemberInfo memberInfo, IParentDrawer parent, GUIContent label, bool readOnly, Func<int, object[],bool> validateKey)
		{
			...
			result.Setup(value, keyType, valueType, memberInfo, parent, label, readOnly, validateKey);
			result.LateSetup();
			return result;
		}
		...
		public override void SetupInterface(object setValue, Type setValueType, LinkedMemberInfo setMemberInfo, IParentDrawer setParent, GUIContent setLabel, bool setReadOnly)
		{
			...
			if(setParent != null)
			{
				var parentType = setParent.Type;
				if(!TryGetMemberTypes(parentType, out entryKeyType, out entryValueType))
				{
					#if DEV_MODE
					Debug.LogWarning(ToString(setLabel, setMemberInfo)+" - Setting memberTypes to System.Object because TryGetMemberTypes failed for type "+StringUtils.ToStringSansNamespace(parentType)+".");
					#endif
			...
			{
				#if DEV_MODE
				Debug.LogWarning("DictionaryEntry.Setup called with no parent. Determining key and value types might not be possible...");
				#endif
			...
			Setup(dictionaryEntry, entryKeyType, entryValueType, setMemberInfo, setParent, setLabel, setReadOnly, null);
		}
		...
		private void Setup(DictionaryEntry setValue, Type entryKeyType, Type entryValueType, LinkedMemberInfo setMemberInfo, IParentDrawer setParent, GUIContent setLabel, bool setReadOnly, [CanBeNull]Func<int, object[],bool> setValidateKey)
		{
			...
			Debug.Log("DictionaryEntryDrawer<"+ StringUtils.ToString(setKeyType) +","+ StringUtils.ToString(setValueType) + "> created with drawInSingleRow="+ StringUtils.ToColorizedString(drawInSingleRow)+", value="+StringUtils.ToString(Value));
			#endif
		}
