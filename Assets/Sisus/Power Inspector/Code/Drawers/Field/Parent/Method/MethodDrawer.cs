...
namespace Sisus
{
	...
	public class MethodDrawer : ParentFieldDrawer<object>
	{
		...
		protected virtual void Setup([NotNull]LinkedMemberInfo setMemberInfo, [CanBeNull]IParentDrawer setParent, [CanBeNull]GUIContent setPrefixLabel, [CanBeNull]GUIContent setButtonLabel, bool setReadOnly)
		{
			...
			{
				setButtonLabel = preferences.labels.invokeMethod;
			}
		...
		protected object GetValue(int index, [NotNull]out string error)
		{
			...
			if(isGeneric)
			{
				...
				catch(Exception e)
				{
					...
					error = e.ToString();
					//InspectorUtility.ActiveInspector.Message("Invoke Method "+e, null, MessageType.Error);
					return DefaultValue();
				}
			}
			else
			{
				...
				{
					error = e.ToString();
					//InspectorUtility.ActiveInspector.Message(e.ToString());
					return DefaultValue();
				}
			...
			{
				#if DEV_MODE && DEBUG_INVOKE
				Debug.Log("Invoking method "+methodInfo.Name+" with "+(parameters == null ? "" : parameters.Length+" ")+"params="+StringUtils.ToString(parameters)+", hasParameters="+StringUtils.ToColorizedString(hasParameters));
				#endif
			...
			//update result Drawer with value returned by method
			if(hasReturnValue)
			{
			...
			#if CALL_ON_VALIDATE
			else if(!methodInfo.IsStatic && methodInfo.GetCustomAttributes(typeof(PureAttribute), false).Length == 0)
			{
				...
				else if(Event.current.button == 1)
				{
					...
					if(hasReturnValue)
					{
						...
						if(UnityObjectExtensions.IsUnityObjectOrUnityObjectCollectionType(Type))
						{
							...
							//menu.Add(Menu.Item("Inspect Return Value", () => Invoke(true, false, false, false)));
							//menu.Add(Menu.Item("Peek Return Value", () => Invoke(true, false, false, false)));
						}
					}
