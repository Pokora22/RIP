...
namespace Sisus
{
	...
	public abstract class ColorBaseDrawer<TColor> : PrefixControlComboDrawer<TColor> where TColor : struct
	{
		...
		public override Part MouseoveredPart
		{
			get
			{
				return eyeDropToolMouseovered ? Part.Picker : base.MouseoveredPart;
			}
		}
...
if(valueUnapplied != ToColor(setValue)) { Debug.Log(ToString()+".DoSetValue("+setValue+", applyToField="+applyToField+")"); }
		...
		public override TColor DrawControlVisuals(Rect position, TColor inputValue)
		{
			...
			{
				#if UNITY_EDITOR
				// try to figure out whether should apply the value last selected via the ColorPicker
				...
				// the value is by double clicking an entry in the color picker, or by click off-screen
				// of the color picker to close it.
				var inspector = InspectorUtility.ActiveInspector;
				inspector.RefreshView();
				
				// re-focus the EditorWindow that was used to open the TColor Picker - it always loses
				// focus to the TColor Picker window when it's opened.
		...
		protected override void BuildContextMenu(ref Menu menu, bool extendedMenu)
		{
			...
			menu.Add("Set Value/Clear", SetValueFromMenu, Color.clear);
			menu.Add("Set Value/White", SetValueFromMenu, Color.white);
			menu.Add("Set Value/Black", SetValueFromMenu, Color.black);
			menu.Add("Set Value/Gray", SetValueFromMenu, Color.gray);
			menu.Add("Set Value/Red", SetValueFromMenu, Color.red);
			menu.Add("Set Value/Yellow", SetValueFromMenu, Color.yellow);
			menu.Add("Set Value/Blue", SetValueFromMenu, Color.blue);
			menu.Add("Set Value/Green", SetValueFromMenu, Color.green);
			menu.Add("Set Value/Cyan", SetValueFromMenu, Color.cyan);
			menu.Add("Set Value/Magenta", SetValueFromMenu, Color.magenta);
			
			menu.Add("Set Alpha/Opaque", SetAlphaFromMenu, 1f);
			menu.Add("Set Alpha/Transparent", SetAlphaFromMenu, 0f);
