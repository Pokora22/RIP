...
namespace Sisus
{
	...
	public abstract class PolymorphicDrawer : ParentFieldDrawer<object>, IDraggablePrefixAffectsMember
	{
		...
		protected override void OnCachedValueChanged(bool applyToField, bool updateMembers)
		{
			...
			#if DEV_MODE
			Debug.Log(Msg(ToString()+".OnCachedValueChanged with instanceTypeWas=", typeWas, ", instanceType=", instanceType));
			#endif
		...
		protected override void DoBuildMembers()
		{
			...
			if(typeForValue == null)
			{
				if(CanBeUnityObject)
				{
					var referenceField = ObjectReferenceDrawer.Create(null, memberInfo, GetTypeForObjectReferenceField(), this, GUIContent.none, AllowSceneObjects(), ReadOnly);
					if(DrawToggleNullButton)
					{
		...
		{
			var type = Type;
			return !type.IsAbstract && !Types.UnityObject.IsAssignableFrom(type) ? Types.UnityObject : type;
		}
		...
		protected override void GetDrawPositions(Rect position)
		{
			...
			else
			{
				...
				if(DrawToggleNullButton)
				{
					...
					toggleNullButtonPosition = controlRect;
				}
			}
		}
