...
namespace Sisus
{
	...
	public abstract class ParentFieldDrawer<TValue> : FieldDrawer<TValue>, IParentDrawer
	{
		...
		public virtual void OnMemberValueChanged(int memberIndex, object memberValue, LinkedMemberInfo memberLinkedMemberInfo)
		{
			...
			if(memberInfo != null && memberLinkedMemberInfo != null && !memberLinkedMemberInfo.ParentChainIsBroken && memberInfo.CanRead)
			{
			...
			if(tryToUpdateFromField && UpdateCachedValueFromField())
			{
				#if DEV_MODE && DEBUG_ON_MEMBER_VALUE_CHANGED
		...
		public override void OnMouseUpAfterDownOverControl(Event inputEvent, bool isClick)
		{
			...
			Debug.Log("isClick="+StringUtils.ToColorizedString(isClick) + ", Foldable=" + StringUtils.ToColorizedString(Foldable) + ", foldArrowMouseovered=" + StringUtils.ToColorizedString(foldArrowMouseovered) + ", wasSelectedOnClickStart=" + StringUtils.ToString(wasSelectedOnClickStart));
			#endif

			if(isClick && Foldable)
			{
				if(foldArrowMouseovered || wasSelectedOnClickStart || InspectorUtility.Preferences.changeFoldedStateOnFirstClick)
				{
