...
namespace Sisus
{
	...
	public abstract class RangeDrawer<TValue> : NumericDrawer<TValue> where TValue : IConvertible, IComparable
	{
		...
		{
			Debug.Log(ToString()+".OnMouseUpAfterDownOverControl with mouseDownOverSubPart="+ mouseDownOverSubPart);
		...
		private void OnMouseoverSlider()
		{
			...
			Vector2 drawAreaOffsetDifference = currentDrawAreaOffset - localDrawAreaOffset;

			var visualSliderRect = SliderPosition;
			//visualSliderRect.x += offset.x;
			//visualSliderRect.y += offset.y;

			var effectiveSliderRect = SliderClickableAreaRect;
			//effectiveSliderRect.x += offset.x;
			//effectiveSliderRect.y += offset.y;
			...
			if(diff > 6)
			{
				...
				tooltipRect.height -= 2f;
				var tooltipWidth = DrawGUI.prefixLabel.CalcSize(tooltip).x + 3f;
				tooltipRect.x = mousePos.x - tooltipWidth * 0.5f;
				...
				effectiveSliderRect.x = tooltipRect.x;
				//effectiveSliderRect.x -= drawAreaOffsetDifference.x;
				...
				GUI.Label(effectiveSliderRect, " ", "Icon.Connector");
