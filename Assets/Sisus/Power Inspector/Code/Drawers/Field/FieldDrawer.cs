...
namespace Sisus
{
	...
	public abstract class FieldDrawer<TValue> : BaseDrawer, IFieldDrawer
	{
		...
		public virtual TValue Value
		{
			...
			set
			{
				DoSetValue(value, CanWriteToField, true);
			}
		}
		...
		public override Type Type
		{
			get
			{
				...
				Debug.Assert(memberInfo != null || value != null, ToString()+".Type called with both memberInfo and value null!");
				#endif
				return DrawerUtility.GetType(memberInfo, value);
			}
		}
		...
		/// </value>
		protected UnityEditor.PropertyModification[] PropertyModifications
		{
			get
			{
				return UnityEditor.PrefabUtility.GetPropertyModifications(UnityObject);
			}
		}
		#endif
		...
		protected virtual void Setup([CanBeNull]TValue setValue, [NotNull]Type setValueType, [CanBeNull]LinkedMemberInfo setMemberInfo, [CanBeNull]IParentDrawer setParent, [CanBeNull]GUIContent setLabel, bool setReadOnly)
		{
			...
			showInInspectorIf = setMemberInfo != null ? setMemberInfo.GetAttribute<IShowInInspectorIf>() : null;
			#if DEV_MODE
			if(showInInspectorIf != null && setParent != null) { Debug.Log(ToString()+ " has showInInspectorIf with setParent=" + setParent+ ", setParent.Type="+ setParent.Type.Name+ ", setParent.GetValue()="+StringUtils.ToString(setParent)); }
			#endif
			passedLastShowInInspectorIfTest = showInInspectorIf == null || setParent == null || showInInspectorIf.ShowInInspector(setParent.Type, setParent.GetValue(), setMemberInfo == null ? null : setMemberInfo.MemberInfo);
			...
			if(readOnly || !canWriteToField)
			{
				...
				{
					Debug.Log(StringUtils.ToColorizedString(ToString(), " readOnly=", readOnly, ", setReadOnly=", setReadOnly, ", canWriteToField=", canWriteToField, ", setMemberInfo=", setMemberInfo, ", setMemberInfo.CanWrite=", setMemberInfo.CanWrite, ", setParent.MemberInfo=", setParent == null ? null : setParent.MemberInfo));
				}
				else
				{
					Debug.Log(StringUtils.ToColorizedString(ToString(), " readOnly=", readOnly, ", setReadOnly=", setReadOnly, ", canWriteToField=", canWriteToField, ", setMemberInfo=", null, ", setParent.MemberInfo=", setParent == null ? null : setParent.MemberInfo));
				}
			}
			#endif
		...
		public override bool SetValue(object setValue)
		{
			try
			{
				return DoSetValue((TValue)setValue, CanWriteToField, true);
			}
			catch(InvalidCastException e)
			{
		...
		protected virtual bool DoSetValue(TValue setValue, bool applyToField, bool updateMembers)
		{
			...
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(!applyToField || CanWriteToField, ToString()+".DoSetValue called with applyToField="+StringUtils.True+" but CanWriteToField="+StringUtils.False);
			#endif
			...
			{
				value = setValue;
				OnCachedValueChanged(applyToField, updateMembers);
		...
		protected bool UpdateCachedValueFromField()
		{
			if(!CanReadFromFieldWithoutSideEffects)
			{
				return false;
		...
		{
			return GetCopyOfValuesInternal() as object[];
		}
		...
		public override void OnSiblingValueChanged(int memberIndex, object memberValue, [CanBeNull] LinkedMemberInfo memberLinkedMemberInfo)
		{
			...
			if(showInInspectorIf != null && parent != null)
			{
				bool set = showInInspectorIf.ShowInInspector(parent.Type, parent.GetValue(), memberInfo == null ? null : memberInfo.MemberInfo);
		...
		protected virtual void OnCachedValueChanged(bool applyToField, bool updateMembers)
		{
			...
			Debug.Log(StringUtils.ToColorizedString(ToString(), ".OnCachedValueChanged: ", StringUtils.ToString(value), " with applyToField=", applyToField, ", updateMembers=", updateMembers, ", OnValueChanged = ", StringUtils.ToString(OnValueChanged), ", inactive=", inactive));
			#endif
		...
		protected override void BuildContextMenu(ref Menu menu, bool extendedMenu)
		{
			...
			if(canWrite)
			{
				...
				if(CanWriteToField)
				{
					...
					{
						menu.AddSeparatorIfNotRedundant();
						menu.Add("Revert Value To Prefab", RevertValueToPrefab);
					}
					#endif
		...
		/// </summary>
		private void RevertValueToPrefab()
		{
			var prop = memberInfo.SerializedProperty;

			UndoHandler.RegisterUndoableAction(UnityObjects, StringUtils.Concat("Revert ", GetFieldNameForMessages(), " Value To Prefab"));
		...
		private void GenerateValidationOverrideFromAttributes()
		{
			...
			var notNull = memberInfo.GetAttribute<NotNullAttribute>();
			if(notNull != null)
			{
