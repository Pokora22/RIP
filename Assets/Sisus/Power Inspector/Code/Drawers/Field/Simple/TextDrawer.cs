...
namespace Sisus
{
	[Serializable, DrawerForField(typeof(string), false, true), DrawerForPropertyDrawer(true, typeof(TextAreaAttribute), typeof(string)), DrawerForPropertyDrawer(true, typeof(DelayedAttribute), typeof(string))]
	public class TextDrawer : PrefixControlComboDrawer<string>, ITextFieldDrawer, IPropertyDrawerDrawer
	{
		...
		private bool textArea; // if true text field is drawn below the prefix, utilizing the whole width of the inspector view
		...
		{
			bool setTextArea = attribute is TextAreaAttribute || (setMemberInfo != null && setMemberInfo.GetAttribute<TextAreaAttribute>() != null);
		...
		private void UpdateDynamicHeightWhenReady()
		{
			...
			// If the inspector is null the drawer has probably been disposed.
			if(inspector == null)
			{
				#if DEV_MODE
				Debug.LogWarning("UpdateDynamicHeightWhenReady - aborting because Inspector was null");
				#endif
				return;
			}
			...
			{
				inspector.OnNextLayout(UpdateDynamicHeightWhenReady);
				return;
			}
		...
		public override bool DrawBody(Rect position)
		{
			...
			if(changed)
			{
				...
				if(!ReadOnly && (MixedContent || !string.Equals(valueWas, setValue, StringComparison.Ordinal)))
				{
					#if DEV_MODE && DEBUG_VALUE_CHANGED
