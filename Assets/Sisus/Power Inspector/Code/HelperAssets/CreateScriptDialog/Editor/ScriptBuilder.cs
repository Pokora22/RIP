...
namespace UnityEditor
{
	internal class ScriptBuilder : IDisposable
	{
		...
		{
			text = scriptPrescription.m_Template;
			...
			// Other replacements
			foreach(var keyAndValue in scriptPrescription.m_StringReplacements)
			{
			...
			if(match.Success)
			{
				...
				if(scriptPrescription.m_Functions != null)
				{
					var includedFunctions = scriptPrescription.m_Functions.Where(f => f.include).ToArray();
