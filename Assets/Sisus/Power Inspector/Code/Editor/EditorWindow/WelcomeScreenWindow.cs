...
namespace Sisus
{
	...
	public class WelcomeScreenWindow : EditorWindow
	{
		...
		private static readonly GUIContent issueTrackerLabel = new GUIContent("Issue\nTracker", "Visit issue tracker to report or track bugs, ask questions and offer your ideas.\n\nYou can also do this using the menu item\nWindow > Power Inspector > Help > Issue Tracker");
		...
		private void OnGUI()
		{
			...
			GUILayout.BeginHorizontal();
			{
				...
				if(GUILayout.Button(issueTrackerLabel, buttonStyle, GUILayout.Width(90f), GUILayout.Height(50f)))
				{
					DrawGUI.ExecuteMenuItem(PowerInspectorMenuItemPaths.IssueTracker);
					GUIUtility.ExitGUI();
				}
