...
namespace Sisus
{
	...
	public abstract class InspectorDrawerWindow : EditorWindow, ISplittableInspectorDrawer
	{
		...
		public static IInspectorDrawer CreateNewWithoutShowing([NotNull] Type windowType, bool addAsTab)
		{
			...
			if(focusedWindow != null && string.Equals(focusedWindow.titleContent.text, "Inspector", StringComparison.Ordinal))
			{
			...
			{
				existingWindow = manager == null ? null : manager.GetLastSelectedInspectorDrawer(windowType) as EditorWindow;
			}
		...
		protected virtual void Setup(Object[] inspect, bool lockView, Vector2 scrollPos, float minWidth = 280f, float minHeight = 130f, float maxWidth = 0f, float maxHeight = 0f)
		{
			...
			{
				messageDispenser = GetMessageDispenser();
			}
			else
			{
				messageDispenser.Setup(this, preferences);
			}
		}
		...
		/// </summary>
		/// <returns></returns>
		protected virtual IEditorWindowMessageDispenser GetMessageDispenser()
		{
			var preferences = GetPreferences();
			...
			{
				return new NotificationMessageDispenser(this, preferences);
			}

			if(displayMethod.HasFlag(MessageDisplayMethod.Console))
			{
				return new ConsoleMessageDispenser(this);
			}
...
//return;
			...
			if(type == EventType.ValidateCommand && string.Equals(e.commandName, "Copy", StringComparison.Ordinal) && selectionRect.y < 1f)
			{
				#if DEV_MODE
...
Debug.Log("Repaint");
		...
		private void OnFocus()
		{
			...
			if(selectedInspector != mainView && (!viewIsSplit || splitView != selectedInspector))
			{
				...
				selectedInspector = mainView;
			}

			// Unity seems to have a system where it restores the previously selected control
			// when a window regains focus. We however don't want this feature, as we currently
			// don't remember the previously selected control, so prevent it.
			if(GUIUtility.hotControl == 0)
			{
				if(selectedInspector != splitView)
				{
					inspectorManager.OnNextLayout(()=>
					{
						if(mainView.FocusedDrawer == null && GUIUtility.hotControl == 0)
						{
				...
				else
				{
					inspectorManager.OnNextLayout(()=>
					{
						if(ViewIsSplit && splitView.FocusedDrawer == null && GUIUtility.hotControl == 0)
						{
							KeyboardControlUtility.KeyboardControl = 0;
						}
					});
		...
		private void OnLostFocus()
		{
			...
			if(inspectorManager != null)
			{
				...
				if(selectedInspector != null && ReferenceEquals(selectedInspector.InspectorDrawer, this))
				{
					inspectorManager.Select(null, InspectorPart.None, ReasonSelectionChanged.LostFocus);

					// This should be safe to do, because this gets called before the mouse events get called on the newly
					// focused window during a click event. Still check that hotControl is 0 just to be sure.
					// UPDATE: This does break drag and drop events if called when GUIUtility.hotControl is not 0!
					if(GUIUtility.hotControl == 0)
					{
						KeyboardControlUtility.KeyboardControl = 0;
		...
		protected static void OnScriptsReloaded()
		{
			...
			if(count > 0)
			{
				...
				{
					var window = instances[n];
					window.nowRestoringPreviousState = true;
				}

				manager.OnNextLayout(()=>
				{
					for(int n = instances.Length - 1; n >= 0; n--)
					{
						var window = instances[n];
						if(!window.SetupDone)
						{
		...
		public void Message(GUIContent message, Object context = null, MessageType messageType = MessageType.Info, bool alsoLogToConsole = true)
		{
			...
			{
				Debug.Log("Won't show message because messageDispenser null: \""+message.text+"\"");
				#if PI_ASSERTATIONS
