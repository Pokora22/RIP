...
namespace Sisus
{
	...
	public static class PowerInspectorMenuItems
	{
		...
		private static void ViewObjectInPowerInspector(MenuCommand command, bool useSplitView)
		{
			...
			if(gameObject != null)
			{
				var inspector = manager == null ? null : manager.LastSelectedActiveOrDefaultInspector(gameObject.scene.IsValid() ? InspectorTargetingMode.Hierarchy : InspectorTargetingMode.Project);
				if(inspector == null)
				{
			...
			{
				var inspector = manager == null ? null : manager.LastSelectedActiveOrDefaultInspector(target.IsSceneObject() ? InspectorTargetingMode.Hierarchy : InspectorTargetingMode.Project);
				if(inspector == null)
				{
					var window = PowerInspectorWindow.CreateNew(false);
					inspector = window.MainView;
