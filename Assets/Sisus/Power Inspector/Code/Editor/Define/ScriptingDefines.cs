	...
	public static class ScriptingDefines
	{
		...
		private static int IndexOf(string scriptingDefineSymbols, string define)
		{
			...
			while(end != -1)
			{
				...
				#endif
				if(string.Equals(scriptingDefineSymbols.Substring(start, end - start), define, StringComparison.Ordinal))
				{
					return start;
				}
			...
			if(string.Equals(scriptingDefineSymbols.Substring(start), define, StringComparison.Ordinal))
			{
				return start;
			}
