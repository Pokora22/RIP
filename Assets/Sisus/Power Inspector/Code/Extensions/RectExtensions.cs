		...
		public static void GetLabelAndControlRects(this Rect fullRect, GUIContent label, out Rect labelRect, out Rect controlRect)
		{
			...
			else
			{
				if(fullDrawAreaWidth - fullRect.width > DrawGUI.ScrollBarWidth)
				{
					...
					{
						labelRect.width = DrawGUI.SingleLetterPrefixWidth; //to do: use label.GetContentSize ? Would be more expensive though
						controlRect.x += DrawGUI.SingleLetterPrefixWidth;
						controlRect.width -= DrawGUI.SingleLetterPrefixWidth;
