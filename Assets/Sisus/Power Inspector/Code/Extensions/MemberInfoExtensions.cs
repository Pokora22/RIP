...
namespace Sisus
{
	public static class MemberInfoExtensions
	{
		...
		public static bool IsInspectorViewable(this FieldInfo field, bool includeHidden, bool showNonSerialized)
		{
			...
			if(field.IsPublic)
			{
				...
				//public fields are shown by default only if the
				//field type has the Serializable attribute
			...
			//non-public fields are not shown by default
			return false;
		}
		...
		private static bool TryGetAttributeDeterminedInspectorVisibility(MemberInfo member, out bool showInInspector)
		{
			showInInspector = true;
			bool result = false;
			int priority = 0;

			//if a field has been marked with the EditorBrowsableAttribute
			//using the Always parameter, display the field even if it's
			//not public
			var atts = member.GetCustomAttributes(true);
			for(int n = atts.Length - 1; n >= 0; n--)
			{
				var att = atts[n];
		...
		{
			return fieldInfo.Name.EndsWith(">k__BackingField");
		}
		...
		public static void GetInspectorViewables(this Type type, ref LinkedMemberHierarchy hierarchy, [CanBeNull]LinkedMemberInfo parentMemberInfo, [NotNull]ref List<LinkedMemberInfo> results, BindingFlags bindingFlags, bool includeHidden, bool includeNonSerializedFields, PropertyVisibility propertyVisibility, MethodVisibility methodVisibility, LinkedMemberParent parentType)
		{
		...
		public static void GetInspectorViewableFields(this Type type, ref LinkedMemberHierarchy hierarchy, [CanBeNull]LinkedMemberInfo parentMemberInfo, [NotNull]ref List<LinkedMemberInfo> results, BindingFlags bindingFlags, bool includeHidden, bool includeNonSerializedFields, LinkedMemberParent parentType)
		{
			bool notUnitySerialized = parentType == LinkedMemberParent.Static || hierarchy.Target == null;
					...
					case Member.Field:
						...
						for(int n = 0, count = fields.Length; n < count; n++)
						{
							...
							if(field.IsInspectorViewable(includeHidden, includeNonSerializedFields))
							{
								...
								results.Add(hierarchy.Get(parentMemberInfo, field, isStatic ? LinkedMemberParent.Static : parentTypeIfNotStatic, isStatic || noSerializedProperty ? null : field.Name));
							}
						}
						break;
					case Member.Property:
						...
						for(int n = 0, count = properties.Length; n < count; n++)
						{
							...
							if(property.IsInspectorViewable(propertyVisibility, includeHidden))
							{
								...
								results.Add(hierarchy.Get(parentMemberInfo, property, isStatic ? LinkedMemberParent.Static : parentTypeIfNotStatic, isStatic || noSerializedProperty ? null : property.Name));
							}
						}
						break;
					case Member.Method:
