...
namespace Sisus
{
	...
	public static class TypeExtensions
	{
		...
		private static void InitalizedOnLoad()
		{
			if(!setupDone)
			{
				Setup();
		...
		private static void Setup()
		{
			...
			for(int a = assemblies.Length - 1; a >= 0; a--)
			{
				...
				if(assemblyName.StartsWith("Unity", StringComparison.Ordinal))
				{
					...
					if(sub.StartsWith(".", StringComparison.Ordinal) || sub.StartsWith("Engine.", StringComparison.Ordinal) || sub.StartsWith("Editor.", StringComparison.Ordinal) || sub.Equals("Editor", StringComparison.Ordinal))
					{
						unitysAssemblies.Add(assembly);
					}
				}
				else if(string.Equals(assemblyName, "Anonymously Hosted DynamicMethods Assembly", StringComparison.Ordinal))
				{
					continue;
				}
		...
		public static Type GetEnumType(string name, string fullName)
		{
			...
			if(enumTypesByName.TryGetValue(name, out keyValuePairs))
			{
				for(int n = keyValuePairs.Length - 1; n >= 0; n--)
				{
					...
					if(string.Equals(pair.Key, fullName, StringComparison.Ordinal))
					{
						return pair.Value;
					}
				}
