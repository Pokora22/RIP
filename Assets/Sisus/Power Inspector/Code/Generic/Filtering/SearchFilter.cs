...
namespace Sisus
{
	...
	public class SearchFilter
	{
		...
		{
			if(string.Equals(setFilterValue, rawInput, StringComparison.Ordinal))
			{
				return false;
			}
							...
							switch(s[0])
							{
									...
									if(inspector != null && filterLower.StartsWith("s:", StringComparison.OrdinalIgnoreCase) && inspector.GetType() != typeof(PreferencesInspector))
									{
										...
										if(classType != null)
										{
											if(classType.IsComponent())
											{
												...
												var sceneInstances = UnityEngine.Object.FindObjectsOfType(classType);
								...
								case 'i': // icons
									if(inspector != null && inspector.GetType() != typeof(PreferencesInspector))
									{
										...
										{
											var icons = new HashSet<UnityEngine.Object>(); //TO DO: reuse existing instance
											...
											for(int n = allTextures.Length - 1; n >= 0; n--)
											{
												...
												if(texture.name.Length > 0 && texture.name.IndexOf(iconFilter, StringComparison.OrdinalIgnoreCase) != -1 && texture.width >= 4f && texture.height >= 4f && texture.width <= 512f && texture.height <= 512f && !string.Equals(texture.name, "Font Texture", StringComparison.Ordinal))
												{
													icons.Add(texture);
												}
											}
											...
											var array = new UnityEngine.Object[count];
											icons.CopyTo(array);
											...
											var drawer = CustomAssetDrawer.Create(array, inspector.State.drawers, inspector, GUIContentPool.Create("Icon Results"));
											{
												...
												{
													customMembers[n] = ObjectReferenceDrawer.Create(array[n], Types.Texture, drawer, GUIContentPool.Create(array[n].name), false, true);
												}
									...
									if(inspector != null && filterLower.StartsWith("g:", StringComparison.OrdinalIgnoreCase) && inspector.GetType() != typeof(PreferencesInspector))
									{
										...
										if(filterText.Length > 0)
										{
											...
											if(found)
											{
												...
												inspector.State.drawers.DisposeMembersAndClearVisibleMembers();
												inspector.State.inspected = ArrayPool<UnityEngine.Object>.ZeroSizeArray;
