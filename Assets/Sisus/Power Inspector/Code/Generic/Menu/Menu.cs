	...
	public class Menu
	{
		...
		public void Add(ItemInfo menuItem)
		{
			if(!menuItem.IsSeparator)
			{
				...
				{
					if(string.Equals(items[n].Text, menuItem.Text, StringComparison.Ordinal))
					{
		...
		public void MoveItemToBottom(string itemLabel)
		{
			...
			{
				var item = items[n];
				if(string.Equals(item.Text, itemLabel, StringComparison.Ordinal))
				{
					RemoveAt(n);
					Add(item);
					return;
		...
		public int IndexOf(string label)
		{
			for(int n = Count - 1; n >= 0; n--)
			{
				if(string.Equals(items[n].Text, label, StringComparison.Ordinal))
				{
					return n;
				}
			}
