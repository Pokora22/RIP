...
namespace Sisus
{
	public static class CustomEditorUtility
	{
		...
		public static Dictionary<Type, Type> DecoratorDrawersByType
		{
			get
			{
				if(decoratorDrawersByType == null)
				{
					...
					GetDrawersByInheritedInspectedTypesFromAttributes<CustomPropertyDrawer>(decoratorDrawerTypes, typeField, useForChildrenField, ref decoratorDrawersByType);
				}
		...
		/// <param name="drawerTypes"> List of PropertyDrawer, DecoratorDrawer or Editor types. </param>
		/// <param name="inspectedTypeField"> FieldInfo for getting the inspected type. </param>
		...
		private static void GetDrawersByInspectedTypeFromAttributes<TAttribute>([NotNull]Type[] drawerTypes, [NotNull]FieldInfo inspectedTypeField, [NotNull]ref Dictionary<Type,Type> drawersByInspectedType) where TAttribute : Attribute
		{
			...
			for(int n = drawerTypes.Length - 1; n >= 0; n--)
			{
				...
				if(!drawerType.IsAbstract)
				{
					...
					for(int a = attributes.Length - 1; a >= 0; a--)
					{
						...
						var inspectedType = inspectedTypeField.GetValue(attribute) as Type;
		...
		private static void GetDrawersByInheritedInspectedTypesFromAttributes<TAttribute>(Type[] drawerOrEditorTypes, FieldInfo targetTypeField, FieldInfo useForChildrenField, ref Dictionary<Type,Type> addEditorsByType) where TAttribute : Attribute
		{
			for(int n = drawerOrEditorTypes.Length - 1; n >= 0; n--)
			{
				...
				if(!drawerType.IsAbstract)
				{
					var attributes = drawerType.GetCustomAttributes(typeof(TAttribute), true);
					for(int a = attributes.Length - 1; a >= 0; a--)
					{
						var attribute = attributes[a];
						if(attribute != null)
						{
							bool useForChildren = (bool)useForChildrenField.GetValue(attribute);
							if(useForChildren)
							{
								...
								for(int t = extendingTypes.Length - 1; t >= 0; t--)
								{
									...
									if(!extendingType.IsAbstract)
									{
										...
										{
											addEditorsByType[extendingType] = drawerType;
										}
									}
								}
							}
