	...
	public static class HierarchyUtility
	{
		...
		public static bool TryGetNextOfType([NotNull]Component component, [CanBeNull]out Component result)
		{
			...
			if(!component.IsSceneObject())
			{
				...
				var root = component.transform.root;
				if(TryGetNextOfType(root, component, type, ref result))
				{
					if(result != null)
					{
						return true;
			...
			for(int s = 0, count = SceneManager.sceneCount; s < count; s++)
			{
				...
				for(int g = 0, gcount = getRoot.Count; g < gcount; g++)
				{
					if(TryGetNextOfType(getRoot[g].transform, component, type, ref result))
					{
						if(result != null)
						{
							return true;
						...
						getRoot.Clear();
						return TryGetFirstOfType(type, out result) && result != component;
					}
				}
				getRoot.Clear();
			}
		...
		private static bool TryGetNextOfType(Transform currentTransform, Component target, Type type, ref Component result)
		{
			currentTransform.gameObject.GetComponents(type, getComponents);
			int targetIndex = getComponents.IndexOf(target);
			if(targetIndex != -1)
			{
				...
				for(int n = 0, count = currentTransform.childCount; n < count; n++)
				{
					if(TryGetFirstOfType(currentTransform, type, ref result))
					{
						return true;
					}
				}
			...
			for(int n = 0, count = currentTransform.childCount; n < count; n++)
			{
				if(TryGetNextOfType(currentTransform.GetChild(n), target, type, ref result))
				{
					if(result != null)
					{
						return true;
		...
		private static bool TryGetFirstOfType(Transform currentTransform, Type type, ref Component result)
		{
			result = currentTransform.gameObject.GetComponent(type);
			return result != null;
		}
