...
namespace Sisus
{
	...
	public class InspectorPreferences : ScriptableObject
	{
		...
		[Tooltip("If true, Power Inspector will display all public fields by default, even if Unity is unable to serialize them.\nWhen set to false, you will need to manually expose all fields using attributes such as ShowInInspector.\nWhen set to true, you will instead need to manually hide all public non-serialized fields you don’t want to be shown using attributes like HideInInspector.")]
		public bool showNonSerializedFields = false;
		...
		public void Setup()
		{
			...
			converters.Clear(); //remove existing converters, i.e. VectorCoverter (we'll be using the custom RectJsonConverter instead)
			...
			//converters.Add(new QuaternionConverter()); //seems to serialize eulerAngles property for some reason. I don't think that is needed for lossless serialization??
