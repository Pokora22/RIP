...
namespace Sisus
{
	public class InspectorStyles
	{
		...
		public readonly GUIStyle WarningBox;
		public readonly GUIStyle ErrorBox;
		...
		public InspectorStyles([NotNull]GUISkin skin)
		{
			...
			//"OL Ping" not found warning seems to be present when using dark skin, so disabling this for now.
			#if DEV_MODE //UNITY_2018_3_OR_NEWER
			PingedHeader = new GUIStyle(skin.GetStyle("OL Ping"));
			...
			#else
			PingedHeader = new GUIStyle(Label);
			#endif
			...
			WarningBox = skin.GetStyle("WarningBox");
			ErrorBox = skin.GetStyle("ErrorBox");
			...
			WhiteTexture = new GUIStyle(skin.box)
			{
				stretchHeight = true,
				stretchWidth = true,
