		...
		public virtual bool OnKeyboardInputGiven(Event inputEvent, KeyConfigs keys)
		{
			...
			switch(inputEvent.keyCode)
			{
				...
				case KeyCode.DownArrow:
					DrawGUI.Use(inputEvent);
					GUI.changed = true;
					mainView = inspector.InspectorDrawer.MainView;
					var nextDown = mainView.State.drawers.GetNextSelectableDrawerDown(0, null);
					if(nextDown != null)
					{
						inspector.Manager.Select(mainView, InspectorPart.Viewport, nextDown, ReasonSelectionChanged.SelectControlDown);
						return true;
					}
