...
namespace Sisus
{
	...
	public abstract class Inspector : IInspector
	{
		...
		public bool IsBelowViewport(float verticalPoint)
		{
			...
			//return verticalPoint > state.ScrollPos.y + state.WindowRect.height;
			//return verticalPoint > DrawGUI.ActiveScrollViewScrollPosition.y + state.WindowRect.height;
			return verticalPoint > DrawGUI.ActiveScrollViewScrollPosition.y + Screen.height;
		}

		/// <inheritdoc/>
		public bool IsOutsideViewport(Rect rect)
		{
			float start = rect.yMin;
			float end = rect.yMax;
		...
		public void OnSelectionChange()
		{
			...
			{
				RebuildDrawersIfTargetsChanged();
			}
		}
			...
			if(count > 0)
			{
				...
				if(count > 1 && UserSettings.MergedMultiEditMode)
				{
					if(isGameObject)
					{
						...
						#endif
						{
							created = GameObjectDrawer.Create(ArrayPool<Object>.Cast<GameObject>(targets), state.drawers, this);
						}
					}
					else
					{
			...
			switch(e.button)
			{
				case 0:
				{
					...
					if(mouseovered != null)
					{
						...
						{
							#if DEV_MODE && DEBUG_ON_MOUSE_DOWN
							Debug.Log(ToString()+" OnClick of "+mouseovered+" returned "+StringUtils.True);
							#endif
							GUI.changed = true;
						}
		...
		protected bool DrawViewport()
		{
			...
			if(selected && !mouseDownInfo.NowReordering)
			{
				...
				{
					if(selectedControl != null && manager.IsFocusedButNotSelected(selectedControl))
					{
					...
					{
						multiSelected[n].DrawSelectionRect();
					}
				}
			}
		...
		public void OnValidateCommand(Event e)
		{
			...
			if(string.Equals(e.commandName, "Find", StringComparison.Ordinal))
			{
